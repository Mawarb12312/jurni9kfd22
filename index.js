...
textActivity.push(Prefix + "help for commands");
textActivity.push("Using Prefix: " + Prefix);
...
client.on("messageCreate", async (message) => {
  ...
  if (message.content.startsWith(Prefix)) {
    const MT = await mt
      ...
      .catch(console.error);
    if (!message.content.includes("setmt")) {
      if (MT) return message.reply("Bot Maintenance");
    }
    ...
    if (command.accessableby == "admin") {
      ...
      list?.members.cache.forEach((member) => {
        console.log(member);
        if (member.id == Owner) found = true;
      });
      }
      if (!found) {
        message.reply("Owner Not In The Server, Can't Use Admin Commands");
      }
    } else {
      command.run(client, message, args);
    }
